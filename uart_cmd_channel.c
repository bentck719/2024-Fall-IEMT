...
void process_uart_cmd_channel(UINT8 bFlag)
{
	...
	while (xyz[0] == '\r' || xyz[0] == '\n')
 	...
 	else if (strncmp(xyz, "write", 5) == 0)
 	{
    	...
    	// Set Buffer
    	memset((byte *)(FLASH_MEM_BASE), bPATTERN, 8192);
        pSRamBuffer[0] = 0;
	...
	else if (strncmp(xyz, "protectEnable", 14) == 0)
    {
        protect = TRUE;
    }
	else if (strncmp(xyz, "protectDisable", 15) == 0)
    {
        protect = FALSE;
    }
 	else if (strncmp(xyz, "read", 4) == 0)
 	{
        ...
        for( i = 0 ; i <= 255 ; i++ ) 
		{
            UARTprintf(" UART pBuf[%d] 0x %08X %08X %08X %08X\n", i, pSRamBuffer[4*i], pSRamBuffer[4*i+1], pSRamBuffer[4*i+2], pSRamBuffer[4*i+3]);
        }
